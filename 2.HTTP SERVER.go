// HTTP Server
// Introduction
// In this example you will learn how to create a basic HTTP server in Go. First,
//  let’s talk about what our HTTP server should be capable of. A basic HTTP server has a few key jobs to take care of.
...
func main() {
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) { //Process dynamic requests (istilahnya ini route yang dijalankan)
		fmt.Fprintf(w, "Welcome to my website!")
	})

	fs := http.FileServer(http.Dir("static/"))
	http.Handle("/static/", http.StripPrefix("/static/", fs)) //Serving static assets JavaScript, CSS and images,

	http.ListenAndServe(":80", nil) //Accept connections
}

// go run 2.HTTP SERVER.go
